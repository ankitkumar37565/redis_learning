...
app.get("/getAge", async (req, res) => {
  try {
    ...
    if (cachedResult) {
      ...
      // await redisClient.set(req.query.name,JSON.stringify(response.data))
      // await redisClient.lpush('names',JSON.stringify(response.data))
      // await redisClient.rpush('names',JSON.stringify(response.data)) // push at the top of list
      // await redisClient.rpop('names')//drop item from list
      // await redisClient.del('names')//delete a list
      return res.status(200).send({
        success: true,
